---
name: eeat
description: Audit a target's Experience, Expertise, Authoritativeness, and Trust by dispatching three parallel sub-agents that act as Google Search Quality Raters against a page-type-aware checklist. Produces a consensus score, page-quality rating, evidence-backed issues, and prioritized remediation. Targets the project wiki or any public URL.
---
...
- `skills/eeat/references/checklist.md` — page-type-aware E-E-A-T checklist (the contract for every rating).
...
3. **Reputation lookup is mandatory in URL mode.** If a rater cannot run `WebSearch` against `reputation_query`, the engine will cap Authoritativeness numeric score at 50 and gate `reputation_only_self_published`. Do not fake reputation.
...
7. **Present results** to the user: `page_type`, criteria applied/skipped, score (0–100), page_quality, score per pillar, top structured issues, remediation items, and gate/divergence flags. The consolidated narrative IS the analysis — present it as the final word, not as a synthesis of three opinions.
...
- `score` is a number 0–100, derived from applicable criteria only; `not_applicable` is excluded from denominators.
...
- `issues` lists page-type-scoped problems; `gate_flags` lists engine gates and `risk_flags` is reserved for divergence signals.
- The user has been shown the page_type, score, page_quality, score per pillar, structured issues, and prioritized remediation list.
