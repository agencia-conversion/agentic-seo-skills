...
- where `<run-dir>` resolves to `project/.context/autoresearch/<run-id>/` when a is provided and exists, otherwise `.context/autoresearch/<run-id>/`.
...
1. **Init**: call `init --problem "<text>" [--mode general|skill-eval] [--max-iter N] [--threshold N] [--plateau N]`. Capture `run_id` and `run_dir`.
...
4. **Loop** (until decision != `continue`): propose ONE variation. The rationale (≤2 sentences) MUST reference the current best by iter number and explain what differs. Score every committed metric (run executable code for `executable`, judge against rubric prose for `judge`, all scores 0-100; binary metrics are 0 or 100). Call `record --run <id> --variation <file> --rationale "<text>" --scores '<json>'`. Stop when the engine returns `stop:threshold`, `stop:plateau`, or `stop:max_iter`.
5. **Finalize**: call `finalize --run <id>`. Present `winner.md` and `summary.md` to the user.
6. **Read-only invariant**: never overwrite project files outside the run directory. Promotion of `winner.md` to a project artifact (e.g. `project/wiki/...`) is a separate, gated step that requires AskUserQuestion approval.
