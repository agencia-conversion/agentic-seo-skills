...
function additionalContext(input) {
  ...
  return [
    ...
    "Sempre que gerar `report.md`, grave em `project/relatorios/<module>/<run-slug>/report.md`, retorne `report_md` e `browser_prompt: { recommended: true, message: \"Posso abrir o Web Companion para você ver o relatório?\" }`; pergunte essa frase no chat antes de abrir qualquer navegador.",
    ...
    "Skills de dados e relatórios — `seo-analysis`, `technical-seo`, `backlink-analysis`, `keyword-research`, `serp-extract`, `internal-links`, `eeat`, `topic-cluster` — devem sempre escrever `report.md` em `project/relatorios/` e oferecer abertura pelo prompt de chat.",
