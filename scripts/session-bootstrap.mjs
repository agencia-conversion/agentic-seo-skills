...
function additionalContext(input) {
  ...
  return [
    ...
    "",
    "## Formato de entrega",
    "Para qualquer entrega substantiva (relatório, análise, conteúdo, brief, auditoria, recomendação): Web Companion primeiro quando fizer sentido (`project-browser` ou um handoff específico como `approve-page`, `approve-briefing`, `pick-cluster`, `review-changes`).",
    "Sempre que gerar `report.md`, siga a skill compartilhada `page-report`, grave em `project/relatorios/<module>/<run-slug>/report.md`, retorne `report_md` e `browser_prompt: { recommended: true, message: \"Posso abrir o Web Companion para você ver o relatório?\" }`; pergunte essa frase no chat antes de abrir qualquer navegador.",
    "Nunca exponha `node scripts/companion.mjs ...` ao usuário; rode você como agente após o ok.",
    "Use blocos Markdown estruturados (`agentic-kpis`, `agentic-chart`, `agentic-table`) com payload YAML `version: 1` para KPIs, gráficos e tabelas nos relatórios; relatórios são editáveis no Companion, mas criação e exclusão seguem bloqueadas em v1.",
    ...
    "Use `project/.agentic-seo/project.json.language` como idioma canônico do projeto; UI e relatórios v1 têm cópia completa para `pt-BR` e `en`, com override explícito por execução quando existir.",
    "Skills de dados e relatórios — `seo-analysis`, `technical-seo`, `backlink-analysis`, `keyword-research`, `serp-extract`, `internal-links`, `eeat`, `topic-cluster` — devem sempre aplicar `page-report`, escrever `report.md` em `project/relatorios/` e oferecer abertura pelo prompt de chat.",
