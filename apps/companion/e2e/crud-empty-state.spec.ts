...
test.describe('CTA Novo conteúdo — empty-state flows', () => {
  ...
  test('empty cluster: CTA mounts ghost input, Enter creates published content', async ({ page }) => {
    // Empty-state CTA must still mount the ghost input row (regression
    // from when rows.length===0 prevented the table from mounting).
    // Inline CTA semantic v2: Enter creates a published content under
    // contents/blog/<slug>.md with the cluster linked via frontmatter.
    ...
    await ghostInput.fill('Crud Empty Published');
    await ghostInput.press('Enter');

    // File on disk reflects the title verbatim and links the cluster.
    const newPath = join(CONTENTS_BLOG_DIR, 'crud-empty-published.md');
    await expect.poll(() => existsSync(newPath), { timeout: 8_000 }).toBe(true);
    const fm = readFrontmatter(newPath);
    expect(fm.title).toBe('Crud Empty Published');
    expect(fm.origin).toBe('blog');
    expect(Array.isArray(fm.clusters) && (fm.clusters as string[]).includes(EMPTY_SLUG)).toBe(true);
    ...
    await expect(table.locator('[data-cluster-row="crud-empty-published"]')).toBeVisible({
      timeout: 8_000,
    });

    // Brain subpage table must mention the new slug (cluster-sync hook).
    const brain = readFileSync(EMPTY_CLUSTER_BRAIN, 'utf8');
    expect(brain).toContain('crud-empty-published');
  });
