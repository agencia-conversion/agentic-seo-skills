...
|---|---|---|---|
| A | `project/clusters/<slug>/cluster.yaml` | Manifest editorial estruturado | Tese, pillar, planned, overrides, evidência |
| B | `project/contents/<origin>/<slug>.md` | Conteúdo publicado + frontmatter | **Afiliação a clusters** (campo `clusters:`) |
...
pillar:                           # obrigatório se status=active; pode ser null em drafting/proposed
...
planned_satellites:               # APENAS keywords sem conteúdo publicado
  - slug: agentes-de-pesquisa-seo
    ...
    role: satellite               # default; quase nunca pillar
    editorial_status: draft       # draft | in-review | approved | published
    note: "considerar como pillar se o autor migrar"

satellite_overrides:              # opcional; só quando conteúdo publicado precisa de
...
- `status: active` exige `pillar.slug` não-nulo. Lint `cluster.pillar.missing` (block) caso contrário.
- `satellite_overrides[<slug>]` válido apenas quando `<slug>` existe como conteúdo publicado que declara este cluster.

## 5. Schema: frontmatter de `contents/<origin>/<slug>.md`
...
role:                             # opcional
  seo-agentico: satellite         # default seria satellite; redundante mas explícito
  inteligencia-artificial: satellite
---
```
...
- `role` opcional. Ausência = `satellite`. Chave para cluster fora de `clusters:[]` = lint `content.role-orphan`.
- `cluster.yaml.pillar.slug` apontando para este conteúdo **força** "Pilar" mesmo se `role:` diz outro — e emite lint `cluster.pillar.divergence`.
- Conteúdo é **pillar de no máximo 1 cluster**. Violação = lint `cluster.unique-pillar` (block).
...
|---|---|---|---|---|---|---|---|
| Pilar | [O que é SEO Agêntico](../../contents/blog/o-que-e-seo-agentico.md) | seo agêntico (1.2k) | Informacional | Publicado | — | 2026-04-20 | — |
| Satélite | [Skills para SEO](../../contents/blog/skills-para-seo.md) | skills para seo (290) | Informacional | Publicado | — | 2026-04-12 | inteligencia-artificial |
...
- Links na coluna "Conteúdo" usam path relativo `../../contents/<origin>/<slug>.md` (gera backlink Obsidian + funciona em Reading mode).
...
1. `cluster.yaml.pillar.slug == <content-slug>` → **Pilar** (emite `cluster.pillar.divergence` se frontmatter discordar).
2. `frontmatter.role[<cluster-slug>]` → valor declarado.
3. Default → `satellite`.
...
2. Carrega frontmatter de todos `contents/**/*.md`. Cache invalidado por `mtime`.
3. Para cada cluster (ou só `<slug>` se `--cluster=`):
   - Determina o conteúdo-pillar via `cluster.yaml.pillar.slug`.
   - Coleta conteúdos publicados onde `clusters:[]` inclui o slug do cluster.
   - Coleta `planned_satellites[]` do YAML.
   - Resolve role para cada linha publicada (§ 9).
   - Para cada conteúdo, calcula coluna "Também em" = `clusters:[] \ [<este-cluster>]`.
   - Aplica `satellite_overrides[]` em colunas relevantes.
...
7. Atualiza `stats.published`, `stats.planned`, `stats.updated` em `cluster.yaml`.
8. Regenera `brain/topic-clusters.md` (índice).
9. Emite lints em `brain/log.md` (`type: lint`).
...
| `content.no-clusters` | warn | Arquivo em `contents/` sem `clusters:[]` | Conteúdo aparece em "órfãos" no painel do índice |
| `content.cluster-fanout` | warn | Conteúdo em >= 4 clusters | Tabela em cada cluster; sinal de tagging frouxo |
| `content.role-orphan` | warn | `role:{<cluster>}` para cluster fora de `clusters:[]` | Ignorado |
| `cluster.pillar.divergence` | warn | `pillar.slug` aponta para conteúdo cujo `role:` diz outro | YAML vence; warning visual |
| `cluster.pillar.missing` | block | `status: active` sem `pillar.slug` | Pre-commit falha |
| `cluster.unique-pillar` | block | Mesmo conteúdo é pillar em >= 2 clusters | Pre-commit falha |
| `cluster.planned-collision` | warn | `planned_satellites[]` colide com conteúdo publicado | Sync migra para `satellite_overrides` + `type: decision` no log |
...
Após `POST /api/project/file` em `contents/` ou `cluster.yaml`:
...
const labels: Record<string, Record<string, string>> = {
  'pt-BR': { role: 'Papel', content: 'Conteúdo', keyword: 'Keyword (vol.)', intent: 'Intenção',
             status: 'Status', action: 'Ação', updated: 'Atualizado', also_in: 'Também em',
             pillar: 'Pilar', satellite: 'Satélite', planned: 'Planejado' },
  'en':    { role: 'Role', content: 'Content', keyword: 'Keyword (vol.)', intent: 'Intent',
             status: 'Status', action: 'Action', updated: 'Updated', also_in: 'Also in',
             pillar: 'Pillar', satellite: 'Satellite', planned: 'Planned' }
};
```
...
- Pillar em 2 clusters → lint block.
...
- Planejado colide com publicado → migra campos para `satellite_overrides` + log `type: decision`.
- Edição concorrente Obsidian↔Companion → mtime check + 409.
- Loop watcher↔hook → fingerprint no-op.
- Cluster ativo sem pillar → block.
...
5. **PR 5: Companion server hook + UI.** `POST /api/project/file` hook + mtime check. `FrontmatterDrawer` multi-select `clusters` + `role`. Bulk reassign na listagem de Conteúdos.
...
- `v1.1` (2026-05-25, plugin 0.2.0) — campos adicionados sem breaking change:
  ...
  - `satellite_overrides[<slug>].editorial_status` e `planned_satellites[<i>].editorial_status` opcionais; valores `draft | in-review | approved | published`. Default: `published` para conteúdo publicado sem override, `draft` para planejado sem override.
