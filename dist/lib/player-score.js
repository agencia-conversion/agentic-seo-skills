...
exports.buildPlayerScoreReport = buildPlayerScoreReport;
...
function parseJsonArg(value, deps) {
    if (!value || value === true || value === "true")
        return {};
    const raw = String(value);
    const candidate = path.isAbsolute(raw) ? raw : path.resolve(deps.rootDir, raw);
    if (fs.existsSync(candidate))
        return deps.readJson(candidate);
    return JSON.parse(raw);
}
function extractSerpTerms(topResults, players, limit = 12) {
    ...
    for (const result of topResults)
        add(`${result.title} ${result.snippet}`, 2);
    for (const player of players)
        add((player.page?.headings || []).map((h) => h.text).join(" "), 1);
...
function scoreQueryRelevance(keyword, player) {
...
async function loadPlayerPage(url, fixtures, deps) {
    ...
    try {
        ...
        return { html: "", status: null, source: url, headers: {}, ok: false, error: String(error.message || error) };
    }
}
async function buildPlayerScoreReport(args, base, deps) {
    const keyword = base.keyword;
    if (!args.target_url && !args.target_domain)
        throw new Error("Player score requires --target-url or --target-domain.");
    ...
    if (!rankedTarget && !seen.has(targetKey))
        players.push({ url: fallbackTargetUrl, url_key: targetKey, host: targetHost, position: null, serp: null, is_target: true });
    ...
    const playerScores = players.map((player) => ({
        ...
        extracted_summary: { title: player.page.title, h1: (player.page.headings || []).find((h) => h.level === "h1")?.text || "", h2_count: player.page.h2_count, word_count: player.page.word_count },
