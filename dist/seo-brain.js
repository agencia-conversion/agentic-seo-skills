...
function slugify(value) {
    const slug = value
        ...
        .slice(0, 80);
    if (!slug)
        throw new CliError("Project slug is empty after normalization.");
    if (slug === "." || slug === ".." || slug.includes("/"))
        throw new CliError("Unsafe project slug.");
    return slug;
}
...
function ensureProject(project) {
    ...
    if (!fs.existsSync(p))
        throw new CliError(`Project not found: ${p}`);
    return p;
}
function mkdirp(p) {
...
function appendLog(project, eventType, title, files, summary, approval) {
...
async function commandProjectInit(args) {
    const name = args._[0];
    if (!name)
        throw new CliError("project-init requires a project name.");
    const slug = slugify(name);
    const p = projectPath(slug);
    for (const dir of ["wiki", "web", "sources", "reports", "artifacts", ".seo-brain"])
...
async function commandTopicCluster(args) {
    ...
    if (!fs.existsSync(analysisFile) && !args.hypothesis_only)
        throw new CliError(`Missing seo-analysis for this seed. Run: bin/seo-brain seo-analysis --project ${project} --keyword "${seed}" or rerun with --hypothesis-only to produce a hypothesis-grade cluster.`);
...
async function commandContentSeo(args) {
    ...
    if (!fs.existsSync(analysisFile) && !args.skip_data)
        throw new CliError(`Missing seo-analysis for this topic. Run: bin/seo-brain seo-analysis --project ${project} --keyword "${keyword}" or rerun content-seo with --skip-data --skip-data-reason "motivo claro" para gerar um briefing sem proveniencia de SERP.`);
...
async function commandAutoresearch(args) {
