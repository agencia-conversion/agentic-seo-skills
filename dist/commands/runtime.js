...
function dataforseoBypassError(context) {
    ...
    return new CliError(`${context.workflow}${subject} requires a recorded reason to proceed without DataForSEO at ${context.step}. ` +
...
function normalizeDataforseoBypassApproval(args, context, mode) {
    ...
    const confirmationText = nonemptyString(args.dataforseo_bypass_confirmation_text, `Registrado automaticamente: seguir sem DataForSEO em ${context.workflow}/${context.step}.`);
...
function brainExcerpts(body, max = 3) {
    return body
        .split(/\r?\n/)
...
async function buildCompetitorEvidence(analysisData, args) {
    ...
    if (top3.length < 3 && !bypassReason)
        throw new CliError('Top 3 competitor evidence requires 3 organic results. Rerun SERP extraction or use --top3-bypass-reason "motivo claro" to record the limitation.');
    ...
    if (failures.length && !bypassReason)
        throw new CliError(`Top 3 competitor evidence failed: ${failures.join("; ")}. Use --top3-bypass-reason to record the limitation.`);
    return {
        generated_at: nowIso(),
...
function buildSkyscraperWordCount(competitorEvidence, args) {
    ...
    if (!valid.length && !args.top3_bypass_reason)
        throw new CliError("Cannot calculate target_words: no valid Top 3 competitor word counts. Use --top3-bypass-reason to record the limitation.");
...
async function buildContentResearchPacket(topic, keyword, topicSlug, keywordSlug, projectDir, args, competitorEvidence) {
    ...
    if (!analysisFile && !args.skip_data)
        throw new CliError(`Missing seo-analysis for this topic. Run seo-analysis for "${keyword}" or rerun with --skip-data --skip-data-reason "motivo claro" to record the limitation.`);
...
function assertBriefReadyForWriting(brief, projectDir) {
    if (!["ready-for-writing", "approved-for-writing", "draft", "reviewed", "checked", "checks-failed", "not_required"].includes(String(brief.draft_status || "")))
        throw new CliError("Briefing is not ready for writing.");
...
function validateContextEvidenceForApproval(brief, projectDir, notes = "") {
    ...
    if (voice?.filled !== false && voice?.filled !== true)
        errors.push("invalid-voice-state");
    if (brief.brief?.outline_capacity?.can_support_target !== true)
        errors.push("outline-cannot-support-target");
    if (errors.length)
        throw new CliError(`Briefing validation blocked: ${Array.from(new Set(errors)).join(", ")}`);
}
...
async function commandContentSeo(args) {
    ...
    if (phase === "brief") {
        ...
        if (!["auto", "manual", "handoff"].includes(approvalMode))
        ...
        if (!analysisPath && !args.skip_data)
            throw new CliError(`Missing seo-analysis for this topic. Run seo-analysis for "${paths.keyword}" or rerun with --skip-data --skip-data-reason "motivo claro" to record the limitation.`);
        if (args.skip_data && !args.skip_data_reason)
            throw new CliError('--skip-data requires --skip-data-reason "motivo claro".');
    ...
    if (phase === "approve") {
        ...
        if (!["approved", "ready", "needs-rewrite", "rejected"].includes(decision))
            throw new CliError("--decision must be approved, ready, needs-rewrite, or rejected.");
...
async function commandAuditSkills(args) {
    ...
    function hasForbiddenRequirement(text) {
        return text.split(/\r?\n/).some((line) => {
