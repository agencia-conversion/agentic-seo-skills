...
Use this workflow when the user provides a strategic document (one or more `.md` files, a Google Doc paste, a brand brief) and asks to seed the brain from that source instead of starting with blank templates. Typical signals: "use this doc as the basis for the brain", "based on this PRD, fill identity and editorial", "import this brief into the project".
...
3. The user has explicitly granted authorship of changes (the seed creates substantive brain content; record `approver: <human-name>` in the log).
...
| Stack observed, technical decisions, SEO technical map | `brain/technology.md` | ONLY observed technical facts of the analyzed site (stack, CMS, framework, headers, JSON-LD, rendering, performance signals). NEVER editorial opinion, market positioning, brand thesis, or company values. Observed: "Next.js + Vercel, SSR, schema Article". Opinion ("we prefer static sites", "modern stack is a differentiator") → route to `identity`, `topic-clusters`, or `contents`, never `technology`. |
| Editorial themes (clusters), thesis per cluster, sub-themes, target audience | `brain/topic-clusters.md` (single `agentic-clusters` table, NO áreas) + `project/clusters/<slug>/cluster.yaml` + `brain/topic-clusters/<slug>.md` subpage | `topic-clusters.md` carries one flat `agentic-clusters` auto-block (no per-área H2, no `agentic-clusters-by-area`). Each cluster is a `cluster.yaml` manifest (with a pre-selected `icon` emoji, NO `area`) plus a subpage `brain/topic-clusters/<slug>.md` with editorial prose (thesis/linhas editoriais) + the `cluster-content` block. |
...
| Conceptual frameworks (e.g., 4 levels of AI adoption) | Best-fit cluster subpage `brain/topic-clusters/<slug>.md` as editorial prose | Or its own brain subpage `brain/<parent>/<concept>.md`. |
...
| Pending research, gaps, "to confirm" notes | `brain/log.md` as `type: ingestion` or `type: lint` | Never leave `gap`, `<preencher>`, `TODO` in brain pages. |

### 2. Detect cluster seeds

If the document lists subtopics (e.g., "Estratégia → Brand-Led Growth, Autoatribuição, Posicionamento"), propose 1 cluster draft per subtopic with `status: hypothesis` and an explicit `bypass` record. There is NO "área" layer: `brain/topic-clusters.md` carries a single flat `agentic-clusters` table (no per-área H2, no `agentic-clusters-by-area`). Cluster drafts go to `project/clusters/<slug>/draft.yaml` + `planejamento.md` (table-format manifests, not prose). Promotion to `cluster.yaml` (with a pre-selected `icon` emoji, NO `area`; `status: active`) plus its subpage `brain/topic-clusters/<slug>.md` requires DataForSEO evidence + the `approve-cluster` handoff (separate skill: `topic-cluster`) — unless running under the Step-0 onboarding direct-write authorization (see `seed-from-site.md`). Do not auto-promote.
...
5. Apply the universal review rules from `brain/review.md`: no AI-slop adjectives, no marketing superlatives without proof, no Conversion-explainer voice, lead first sentence.
...
Append a single consolidated `type: decision` entry to `brain/log.md` summarizing the seed run:

```markdown
## YYYY-MM-DD - Brain seeded from <source name>

- type: decision
- scope: brain/identity.md, brain/topic-clusters.md, brain/voice.md, brain/technology.md, brain/products.md (se aplicável)
- decision: Brain semeado a partir do documento "<source name>". Distribuição: <summary by page>. Cluster drafts criados: <list of slugs>.
- evidence: <source path/URL>, e cada wikilink correspondente
- approver: <user name>
- notes: <pending follow-ups, e.g., "Voz preenchida só nos princípios; vocabulário a evitar fica para a primeira sessão de revisão real.">
```
...
- **Do not invent métricas.** If the source says "Conversion Academy grew 290%", quote it with attribution. If the source has a placeholder like "[encontrar dado]", register the pending data point in `log.md` as `type: ingestion` instead of writing a number.
...
3. `brain/log.md` contains the consolidated seed entry plus any `type: evidence` entries for cited proof.
