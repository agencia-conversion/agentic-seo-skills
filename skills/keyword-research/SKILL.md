...
After producing the YAML/JSON artifact above, also render an editable, human-first Companion Markdown report at `project/relatorios/keyword-research/<seed-or-topic-slug>/report.md`. Use the project language from `project/.agentic-seo/project.json.language` unless the execution has an explicit language override; v1 report copy supports `pt-BR` and `en`. Do not put `# <title>` in the body because the Companion title comes from frontmatter. Use structured fences (`agentic-kpis`, `agentic-chart`, `agentic-table`) for visual modules; new fence bodies must be YAML with `version: 1`, not JSON. Write an executive reading first, keep metrics and limitations in human-readable tables, translate labels/status/nulls, use friendly metric names, and never paste raw provider JSON or object arrays into the visual report body. Keep raw and normalized provider evidence in `source_artifact`, `sources/`, `keywords/`, or `workbench/`. Return `report_md` and `browser_prompt: { recommended: true, message: "Posso abrir o Web Companion para você ver o relatório?" }`, then ask that exact consent line in chat before opening any browser. Conversational previews and quick clarifications stay in chat as prose.
