...
- Log entry `tipo: ingestion` with `aprovador: agent` referencing the source.
- Log entry `tipo: approval` with `aprovador: pendente`, proposing the addition of the expert positioning to `identity.md`, citing the source as evidence and naming the missing public proof as a gap.
- Proposed text written to `project/workbench/brain-keeper/identidade-proposal.md`.
- `brain/identity.md` itself unchanged.
- Lint entry if any wikilink fails to resolve or any pt-BR accent is missing.

Constraints:

- Do not modify the raw source.
- Do not promote the strategic claim into `identity.md` without an approved log entry.
