...
- Preserve Agentic SEO source separation: raw evidence in `project/sources/`, synthesis in `project/workbench/`, public content in `project/contents/`, authorial knowledge in `project/brain/`.
- Authorial brain pages require source evidence and a logged `type: decision` entry in `project/brain/log.md` before any change.
