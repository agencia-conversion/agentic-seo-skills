...
- `websearch`: secondary bypass only. Require `--websearch-confirmed --websearch-reason "<motivo>"`; collect results into `sources/websearch/<slug>.yaml` with shape `{keyword, results: [{title, url, snippet, position, domain}]}`.
...
- When websearch is the provider, leave `keyword_metrics` as `null` and record the explicit bypass reason.
