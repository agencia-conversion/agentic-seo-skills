...
- Keep construction files in `project/workbench/content/<slug>/`; keep draft and review deliverables in `project/artifacts/contents/<slug>/`; write public content to `project/conteudos/<origem>/<slug>.md` only after checks pass. Frontmatter must follow the canonical schema (`title`, `slug`, `published_at`, `source_url`, `origem`, `clusters: [<slug>, ...]`, optional `papel: { <cluster-slug>: pilar | satelite }`). `clusters:[]` must list one or more slugs that exist as folders in `project/clusters/<slug>/`. `area:` (singular) is legacy and only tolerated as fallback.
- Every brief receives `clusters:[]` (one or many) as input. Read each `project/brain/topic-clusters/<slug>.md` referenced to extract the cluster's tese, adjacent satellites (for internal links), and editorial tone inherited. Promotion to public content must update the contents table of each referenced cluster subpage; delegate this resync to `brain-keeper cluster-sync` after copying the content to `conteudos/`.
