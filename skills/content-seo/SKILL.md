...
- Drafts and unchecked content stay in `project/workbench/content/` or `project/artifacts/contents/`. Never publish to `project/content/` with failed or missing checks.
...
**Strong:** "Load the ready `brief.yaml`, preserve source-link rules, write `project/artifacts/contents/<slug>/draft.md`, propagate `consensus_backed` / `brand_backed` and Brain/voice flags to frontmatter, and keep `project/content/` untouched."

**Weak:** "Publish a draft to `project/content/blog/` as final content because it will eventually pass review."
...
**Strong:** "After passed checks, copy the final content to `project/content/<origin>/<slug>.md` with the canonical frontmatter and public-safe links, and append a `type: publication` entry to `project/brain/log.md`."

**Weak:** "Move the draft directly to `project/content/` so the user can review it there."
...
artifacts:
  ...
  deliverables:
    ...
    published: project/content/<origin>/<slug>.md
evidence_gates:
...
Output: "Verify the decision record, DataForSEO or bypass disclosures, Top 3 evidence, and filled `project/brain/voice.md`. Then write only to `project/artifacts/contents/seo-agentico/draft.md` and leave `project/content/` untouched."
...
Output: "Write only the requested draft to `project/artifacts/contents/seo-agentico/draft.md`, record that briefing and voice gates were bypassed by direct user request, avoid invented metrics or proof, and do not promote to `project/content/`."

### Example: Weak Execution

Input: "Write and publish an article about `seo agêntico`."

Output: "Guess SERP intent, draft from memory, add local source paths in the article, and publish to `project/content/`." This is weak because it skips DataForSEO/SERP/Top 3 disclosures, skips the three research sub-agents, treats drafting as publication readiness, violates source-link policy, and hides missing checks.
