...
- Adding proof to `project/brain/editorial.md` (or any other authorial brain page) requires sourced present-state findings and a matching `type: decision` or `type: proof` entry in `project/brain/log.md` with actor, evidence, gaps, and timestamp.
...
**Strong:** "Awards, named clients, certifications, and revenue impact remain gaps because no provided source confirms them. They should not be used in public copy or registered as `type: proof` in the brain."
...
Use `project/workbench/eeat/` for audits, draft synthesis, and unverified strategic work. Use `project/artifacts/` for complete deliverables when the user asks for a shareable report. Add proof to `project/brain/log.md` (`type: proof`) and reference it in `project/brain/editorial.md` only with source-backed evidence and a logged decision.
