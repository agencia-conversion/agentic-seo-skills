...
- `project/workbench/topic-cluster/<seed-slug>.yaml`

## Hard Precondition

`workbench/seo-analysis/<seed-slug>.yaml` must exist before a production-grade cluster is generated. Legacy JSON is readable, but new reports are YAML. If absent, the only legal alternative is `--hypothesis-only`, which produces a cluster with `status: hypothesis` clearly marked in the YAML and in the Wiki entry.
