...
Follow the shared `page-report` contract. The module-specific source artifact is `project/audits/<slug>/report.yaml`; the Companion page is `project/relatorios/technical-seo/<slug>/report.md`. Use visual blocks for score, lost points, severity/status, and "Memória de cálculo" / "Calculation memory"; calculation columns must include stable keys plus `role: weight`, `role: points`, and `role: loss`. Put "Meta e indexabilidade" / "Meta and indexability" immediately after the executive reading, use friendly check names for every visible row, and turn deterministic evidence into readable sentences such as "Foram encontrados 8 exemplos de H2..." rather than JSON/object dumps.
