...
Write only to the target project's `brain/`, `sources/`, and `workbench/`. Adding proof to `project/brain/topic-clusters.md` (or any other authorial brain page) requires source evidence and a matching `type: decision` or `type: evidence` entry in `project/brain/log.md` with `approver: agent` or a human name. Keep unsupported evidence in `project/workbench/eeat/<slug>.md`.
