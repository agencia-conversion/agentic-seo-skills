...
2. approval: show evidence of brain consumption (especially `brain/identity.md` and `brain/voice.md`); if approved, write the draft automatically to `project/artifacts/contents/<slug>/draft.md`.
...
5. `promote`: copy checked, finally approved content to `project/contents/<origem>/<slug>.md` with the canonical frontmatter (`title`, `slug`, `published_at`, `source_url`, `origem`, `area`), and append a `tipo: publication` entry to `project/brain/log.md`.
