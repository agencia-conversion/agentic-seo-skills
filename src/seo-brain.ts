...
function lintContentPublication(projectDir: string): AnyRecord[] {
  ...
  for (const name of fs.readdirSync(contentDir)) {
    ...
    for (const issue of issues) findings.push({ severity: "error", file: `conteudos/${name}`, message: issue });
  }
  return findings;
}
...
function validatePublicContentDraft(text: string, brief: AnyRecord): string[] {
  ...
  for (const link of links) {
    ...
    if (/^(?:\.{1,2}\/|\/Users\/|project\/|workbench\/|wiki\/|sources\/)/i.test(link.href)) issues.push(`non-public link target in public body: ${link.href}`);
...
async function commandTopicCluster(args: AnyRecord): Promise<void> {
  ...
  if (!analysisFile && !args.hypothesis_only) throw new CliError(`Missing seo-analysis for this seed. Run: bin/seo-brain seo-analysis --keyword "${seed}" or rerun with --hypothesis-only to produce a hypothesis-grade cluster.`);
...
function readWikiEvidencePage(projectDir: string, rel: string): AnyRecord {
...
function summarizeContextEvidence(evidence: AnyRecord, projectDir: string, evidencePath: string): AnyRecord {
  return {
    ...
    wiki_pages_read: (evidence.wiki_pages_read || []).map((page: AnyRecord) => ({
      path: page.path,
      title: page.title,
      status: page.status,
...
async function buildContentResearchPacket(topic: string, keyword: string, topicSlug: string, keywordSlug: string, projectDir: string, args: AnyRecord, competitorEvidence: AnyRecord): Promise<AnyRecord> {
  ...
  const analysisData = analysisFile ? readDataFile(analysisFile) : null;
  if (analysisData && analysisData.provider !== "dataforseo" && !(boolArg(args.provider_bypass_confirmed, false) && args.provider_bypass_reason)) throw new CliError("content-seo requires DataForSEO-backed seo-analysis. Use --provider-bypass-confirmed --provider-bypass-reason after explicit user approval to use secondary provider data.");
...
function validateContextEvidenceForApproval(brief: AnyRecord, projectDir: string, notes = ""): void {
  ...
  if (voice?.status !== "approved" && !/\b(voz|voice|tom)\b/i.test(notes)) errors.push("voice-context-not-acknowledged");
