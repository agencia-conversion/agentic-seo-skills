...
const sharedReportModules = require("../../shared/report-modules.js") as {
  ...
  REPORT_BROWSER_PROMPT_MESSAGE: string;
};
// eslint-disable-next-line @typescript-eslint/no-var-requires
const companionRoutes = require("../../shared/companion-routes.js") as {
  companionTargetForPath: (projectFilePath: string, baseUrl?: string) => AnyRecord;
};
...
function restoreSnapshot(file: string, snapshot: string | null): void {
  if (snapshot === null) {
...
function reportColumnKey(value: unknown, index: number): string {
...
type ReportTableColumn = string | { key?: string; label: unknown; role?: string };

function uniqueReportColumnKey(value: unknown, index: number, used: Set<string>): string {
...
function reportCell(value: unknown, locale: unknown): unknown {
  if (value == null) return "";
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") return value;
  return humanEvidence(value, locale);
}

function reportTable(columns: ReportTableColumn[], rows: unknown[][], locale: unknown = "pt-BR"): string {
  const usedKeys = new Set<string>();
  const normalizedColumns = columns.map((column, index) => {
    const isSpec = column && typeof column === "object";
    const label = isSpec ? (column as { label: unknown }).label : column;
    const keySource = isSpec ? ((column as { key?: string }).key || label) : label;
    const role = isSpec ? (column as { role?: string }).role : undefined;
    return {
  ...
  const normalizedRows = rows.map((row) =>
    Object.fromEntries(normalizedColumns.map((column, index) => [column.key, reportCell(row?.[index], locale)]))
  );
  const payload = {
    version: 1,
  ...
  return ["```agentic-table", YAML.stringify(payload, { lineWidth: 0 }).replace(/\s+$/, ""), "```"].join("\n");
}

function markdownList(items: unknown[]): string {
  if (!items.length) return "Nenhum item registrado.";
  return items.map((item) => `- ${String(item ?? "").replace(/\n+/g, " ")}`).join("\n");
}

function markdownCode(value: unknown, language = "json"): string {
  return ["```" + language, typeof value === "string" ? value : JSON.stringify(value ?? {}, null, 2), "```"].join("\n");
}

function reportRunSlug(value: string): string {
  return slugify(value).slice(0, 96);
}

function reportMarkdownPath(projectDir: string, moduleId: string, runSlug: string): string {
  return path.join(projectDir, REPORT_DIR_NAME, moduleId, reportRunSlug(runSlug), "report.md");
}

function browserPrompt(reportMd: string, projectDir: string): AnyRecord {
...
function attachReportPrompt<T extends AnyRecord>(data: T, reportMd: string, projectDir: string): T & { report_md: string; browser_prompt: AnyRecord } {
  const reportRel = path.relative(projectDir, reportMd).replace(/\\/g, "/");
  return {
    ...data,
    report_md: reportRel,
...
function artifactBrowserPrompt(file: string, projectDir: string): AnyRecord {
...
function attachArtifactPrompt<T extends AnyRecord>(data: T, file: string, projectDir: string): T & { browser_prompt: AnyRecord } {
  return {
    ...data,
...
function companionTargetForAbsolute(projectDir: string, file: string): AnyRecord {
...
function renderMarkdownReportContent(payload: AnyRecord): string {
  const rendered = spawnSync(process.execPath, [path.join(ROOT, "scripts", "lib", "markdown-report.mjs")], {
    cwd: ROOT,
    encoding: "utf8",
    input: JSON.stringify(payload),
    maxBuffer: 20 * 1024 * 1024,
  });
  if (rendered.error) throw new CliError(`Markdown report failed: ${rendered.error.message}`);
  if (rendered.status !== 0) throw new CliError(`Markdown report failed: ${rendered.stderr || rendered.stdout || "unknown error"}`);
  return rendered.stdout;
}

function writeMarkdownReportOrThrow(projectDir: string, reportMd: string, payload: AnyRecord): void {
  ...
  if (writer.error) throw new CliError(`Report writer failed: ${writer.error.message}`);
  if (writer.status !== 0) throw new CliError(`Report writer failed: ${writer.stderr || writer.stdout || "unknown error"}`);
}

function completeReportWorkflow<T extends AnyRecord>(
  data: T,
  options: {
    projectDir: string;
    reportMd: string;
...
): T & { report_md: string; browser_prompt: AnyRecord } {
...
function contentPublicationCheckFile(projectDir: string, slug: string): string {
...
function copyDir(src: string, dest: string): void {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    if (entry.isFile() && /\.pt-BR\.(md|template)$/i.test(entry.name)) continue;
...
function formatLogFileRefs(files: string[]): string {
  ...
  return files.map((f) => {
    ...
    if (normalized.startsWith("workbench/") || normalized.startsWith("artifacts/") || normalized.startsWith(`${REPORT_DIR_NAME}/`) || normalized.startsWith("../") || normalized.startsWith("sources/") || normalized.includes(".")) return f;
    return `[[${normalized}]]`;
  }).join(", ");
}
...
function mapEventTypeToType(eventType: string): string {
  ...
  if (lower.includes("errat")) return "correction";
  if (lower.includes("prova") || lower.includes("proof") || lower.includes("evidence")) return "evidence";
  return "decision";
}
...
function appendLog(eventType: string, title: string, files: string[], summary: string, approval: string): void {
  ...
  if (eventType === "approval") lines.push(`- approved_at: ${today()}`);
...
function appendDataforseoBypassLog(title: string, approvals: AnyRecord | AnyRecord[] | null | undefined, files: string[]): void {
  ...
  for (const approval of list) {
    ...
    appendOperationalLog(
      ...
      "decision-recorded",
      `Registrado por ${approval.approver || "agent"}; motivo: ${approval.reason}; timestamp: ${approval.confirmado_em}.`,
    );
  }
}
...
function reportLocale(value?: unknown): ReportLocale {
...
function reportText(locale: unknown, pt: string, en: string): string {
  return reportLocale(locale) === "pt-BR" ? pt : en;
}

function unavailable(locale: unknown): string {
  return reportText(locale, "não informado", "not available");
}

function yesNo(value: boolean, locale: unknown): string {
  return value ? reportText(locale, "sim", "yes") : reportText(locale, "não", "no");
}

function severityLabel(value: unknown, locale: unknown): string {
  const key = String(value || "").toLowerCase();
  const pt: Record<string, string> = { critical: "crítico", error: "erro", warning: "atenção", info: "informativo" };
  const en: Record<string, string> = { critical: "critical", error: "error", warning: "warning", info: "info" };
  return reportLocale(locale) === "pt-BR" ? pt[key] || String(value || "") : en[key] || String(value || "");
}

function statusLabel(value: unknown, locale: unknown): string {
  const key = String(value || "").toLowerCase();
  if (key === "pass" || key === "passed" || key === "true") return reportText(locale, "aprovado", "passed");
  if (key === "fail" || key === "failed" || key === "false") return reportText(locale, "requer atenção", "needs attention");
  return String(value || unavailable(locale));
}

function checkLabel(value: unknown, locale: unknown): string {
...
function repairLabel(checkId: unknown, fallback: unknown, locale: unknown): string {
  ...
  if (reportLocale(locale) === "pt-BR") return pt[id] || String(fallback || unavailable(locale));
  return String(fallback || unavailable(locale));
}

function humanKey(value: string, locale: unknown): string {
  const pt: Record<string, string> = {
...
function humanEvidence(value: unknown, locale: unknown): string {
  if (value == null || value === "") return unavailable(locale);
  ...
  if (Array.isArray(value)) {
    if (!value.length) return reportText(locale, "nenhum item", "no items");
    if (value.every((item) => item && typeof item === "object" && ("level" in item || "text" in item))) {
      const examples = value.slice(0, 5).map((item: AnyRecord) => `${String(item.level || "").toUpperCase()}: ${item.text || unavailable(locale)}`);
  ...
  if (typeof value === "object") {
    const entries = Object.entries(value as AnyRecord).filter(([, v]) => v !== undefined && v !== null && v !== "");
    if (!entries.length) return unavailable(locale);
    return entries.slice(0, 8).map(([key, v]) => `${humanKey(key, locale)}: ${humanEvidence(v, locale)}`).join("; ");
  }
  return String(value);
}

function humanList(items: unknown[], locale: unknown): string {
  if (!items.length) return reportText(locale, "Nenhum item registrado.", "No items recorded.");
  return items.map((item) => `- ${humanEvidence(item, locale).replace(/\n+/g, " ")}`).join("\n");
}

function extractSourceLabel(source: string, extracted?: AnyRecord, localeInput?: unknown): string {
  try {
...
function technicalSeveritySummary(report: AnyRecord): AnyRecord {
  const summary: AnyRecord = { critical: 0, error: 0, warning: 0, info: 0 };
  for (const checkItem of report.checks || []) {
    if (!checkItem.passed) summary[checkItem.severity] = (summary[checkItem.severity] || 0) + 1;
  }
  return summary;
}

function buildTechnicalSeoReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  ...
  const failed = memoryChecks.filter((item: AnyRecord) => Number(item.points_lost) > 0);
  const passedCount = memoryChecks.filter((item: AnyRecord) => item.status === "pass").length;
  const failedCount = memoryChecks.filter((item: AnyRecord) => item.status === "fail").length;
  ...
  const calculationRows = (calc.checks || []).map((item: AnyRecord) => [
  ...
  const priorityRows = findings.map((finding: AnyRecord, index: number) => [
    index + 1,
  ...
  const executive = findings.length
    ? reportText(
        locale,
        `A página auditada chegou a **${report.score}/100 (${report.grade})**. O principal ponto de atenção é **${checkLabel(findings[0].check, locale)}**, com evidência: ${humanEvidence(findings[0].evidence, locale)}.`,
        `The audited page scored **${report.score}/100 (${report.grade})**. The main attention point is **${checkLabel(findings[0].check, locale)}**, with evidence: ${humanEvidence(findings[0].evidence, locale)}.`
      )
    : reportText(
        locale,
        `A página auditada chegou a **${report.score}/100 (${report.grade})** e não teve achados determinísticos pendentes nos checks executados.`,
        `The audited page scored **${report.score}/100 (${report.grade})** and had no pending deterministic findings in the executed checks.`
      );
  return {
    ...
    charts: [
      ...
      },
      {
        ...
        height: "tall",
        data: {
          labels: failed.length ? failed.map((item: AnyRecord) => checkLabel(item.id, locale)) : [reportText(locale, "sem perdas", "no losses")],
          datasets: [{ label: reportText(locale, "Pontos perdidos", "Points lost"), data: failed.length ? failed.map((item: AnyRecord) => item.points_lost) : [0], backgroundColor: REPORT_COLORS.red }],
        },
    ...
    sections: [
      ...
      },
      {
        ...
        body_markdown: reportText(
          locale,
          `O score foi calculado a partir dos pesos dos checks executados. O total de peso foi **${calc.total_weight ?? 0}**, com **${calc.points_awarded ?? 0}** pontos conquistados e **${calc.lost_points ?? 0}** pontos perdidos.\n\n${reportTable([{ key: "check", label: "Check" }, { key: "severity", label: "Severidade" }, { key: "status", label: "Status" }, { key: "weight", label: "Peso", role: "weight" }, { key: "points", label: "Pontos", role: "points" }, { key: "loss", label: "Perda", role: "loss" }, { key: "human_evidence", label: "Evidência humana" }], calculationRows, locale)}`,
          `The score was calculated from the weights of the executed checks. Total weight was **${calc.total_weight ?? 0}**, with **${calc.points_awarded ?? 0}** points earned and **${calc.lost_points ?? 0}** points lost.\n\n${reportTable([{ key: "check", label: "Check" }, { key: "severity", label: "Severity" }, { key: "status", label: "Status" }, { key: "weight", label: "Weight", role: "weight" }, { key: "points", label: "Points", role: "points" }, { key: "loss", label: "Loss", role: "loss" }, { key: "human_evidence", label: "Human evidence" }], calculationRows, locale)}`
        ),
      },
      {
        ...
        body_markdown: reportTable([reportText(locale, "Sinal", "Signal"), reportText(locale, "Leitura humana", "Human reading")], [
          ...
          ["H2", humanEvidence((extracted.headings || []).filter((item: AnyRecord) => item.level === "h2"), locale)],
...
function buildSerpReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  const locale = reportLocale(localeInput || report.language);
  ...
  return {
    ...
    charts: [{
      ...
      type: "bar",
      data: { labels: results.slice(0, 10).map((item: AnyRecord) => item.domain || item.url || reportText(locale, "resultado", "result")), datasets: [{ label: reportText(locale, "Posição", "Position"), data: results.slice(0, 10).map((item: AnyRecord) => item.rank_absolute || item.rank_group || 0), backgroundColor: REPORT_COLORS.blue }] },
    ...
    sections: [
      ...
      { heading: reportText(locale, "Resultados orgânicos", "Organic results"), body_markdown: reportTable([reportText(locale, "Posição", "Position"), reportText(locale, "Título", "Title"), reportText(locale, "Domínio", "Domain"), "URL"], results.slice(0, 20).map((item: AnyRecord) => [item.rank_absolute || item.rank_group || "", item.title || "", item.domain || "", item.url || ""])) },
      { heading: "SERP features", body_markdown: humanList(report.serp_features || [], locale) },
    ],
  };
}

function buildKeywordReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  const locale = reportLocale(localeInput || report.language);
  ...
  return {
    ...
    charts: [{
      ...
      type: "bar",
      height: "tall",
      data: { labels: keywords.slice(0, 20).map((item: AnyRecord) => item.keyword), datasets: [{ label: "Volume", data: keywords.slice(0, 20).map((item: AnyRecord) => item.search_volume || 0), backgroundColor: REPORT_COLORS.blue }] },
      options: { indexAxis: "y", scales: { x: { beginAtZero: true } } },
    }],
    sections: [
      ...
      { heading: "Keywords", body_markdown: reportTable(["Keyword", "Volume", "CPC", reportText(locale, "Competição", "Competition")], keywords.slice(0, 100).map((item: AnyRecord) => [item.keyword, item.search_volume ?? unavailable(locale), item.cpc ?? unavailable(locale), item.competition ?? unavailable(locale)])) },
      { heading: reportText(locale, "Nota", "Note"), body_markdown: report.note || reportText(locale, "Sem nota adicional.", "No additional note.") },
    ],
  };
}

function buildBacklinkReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  const locale = reportLocale(localeInput || report.language);
  ...
  return {
    ...
    charts: [{
      ...
      type: "bar",
      data: { labels: [report.target, ...competitors.map((item: AnyRecord) => item.target)], datasets: [{ label: "Backlinks", data: [report.backlinks || 0, ...competitors.map((item: AnyRecord) => item.backlinks || 0)], backgroundColor: REPORT_COLORS.blue }] },
...
function internalLinkPath(value: unknown, siteScope: string): string {
...
function internalLinkPageText(page: AnyRecord): string {
  ...
  if (typeof page.html === "string") return htmlVisibleText(page.html);
  return "";
}

function internalLinkPageLinks(page: AnyRecord): string[] {
  const links = Array.isArray(page.links) ? page.links : [];
  return links.map((link) => typeof link === "string" ? link : String(link?.href || "")).filter(Boolean);
}

function textSnippet(text: string, needles: string[], locale: unknown): string {
  const clean = text.replace(/\s+/g, " ").trim();
  if (!clean) return reportText(locale, "Sem trecho textual disponível.", "No text excerpt available.");
...
function textBeforeAfter(text: string, needle: string): { before: string; after: string } {
  const clean = text.replace(/\s+/g, " ").trim();
  const term = String(needle || "").trim();
  if (!clean || !term) return { before: "", after: "" };
  const lower = clean.toLowerCase();
  const index = lower.indexOf(term.toLowerCase());
  if (index === -1) return { before: clean.slice(0, 120), after: clean.slice(120, 260) };
  return {
...
function uniqueInternalLinkKey(page: AnyRecord, index: number): string {
  return String(page.id || page.url || page.path || page.title || `page-${index + 1}`);
}

function buildInternalLinksReport(input: AnyRecord, args: AnyRecord, projectDir: string): AnyRecord {
  ...
  if (!rawPages.length) throw new CliError("--pages-file must contain a pages array.");
  const firstUrl = rawPages.map((page: AnyRecord) => page.url || page.path).find(Boolean);
  ...
  if (!siteScope) throw new CliError("Provide --site or include site/site_scope in --pages-file.");
  ...
  if (!targetUrl) throw new CliError("Invalid --target URL/path.");
  const targetHost = targetUrl.host;
  const targetPath = internalLinkPath(target, siteScope);
  const targetPage = rawPages.find((page: AnyRecord, index: number) => {
  ...
  const pages = rawPages.map((page: AnyRecord, index: number) => {
    const pageUrl = internalLinkUrl(page.url || page.path || uniqueInternalLinkKey(page, index), siteScope);
...
function renderInternalLinksReview(report: AnyRecord, locale: unknown): string {
  const lines = [
  ...
  if (!(report.recommendations || []).length) lines.push(reportText(locale, "Nenhuma recomendação pronta para revisão.", "No recommendation ready for review."));
...
function buildInternalLinksReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  const locale = reportLocale(localeInput || report.language);
  const recommendations = report.recommendations || [];
  const blocked = report.blocked_candidates || [];
  const recommendationRows = recommendations.map((item: AnyRecord) => [
  ...
  const blockedRows = blocked.map((item: AnyRecord) => [
...
function buildSeoAnalysisReportPayload(report: AnyRecord, localeInput?: unknown): AnyRecord {
  const locale = reportLocale(localeInput || report.language);
  ...
  return {
    ...
    charts: [{
      ...
      type: "bar",
      data: { labels: top.slice(0, 10).map((item: AnyRecord) => item.domain || item.url || reportText(locale, "resultado", "result")), datasets: [{ label: reportText(locale, "Posição", "Position"), data: top.slice(0, 10).map((item: AnyRecord) => item.position || 0), backgroundColor: REPORT_COLORS.blue }] },
      options: { scales: { y: { beginAtZero: true, reverse: true } } },
    }],
    sections: [
      ...
      { heading: reportText(locale, "Top resultados", "Top results"), body_markdown: reportTable([reportText(locale, "Posição", "Position"), reportText(locale, "Título", "Title"), reportText(locale, "Domínio", "Domain"), "URL"], top.slice(0, 20).map((item: AnyRecord) => [item.position, item.title, item.domain, item.url])) },
...
function provenanceRows(provenance: AnyRecord, locale: unknown): unknown[][] {
  const rows: unknown[][] = [];
  if (provenance?.suggestions) rows.push([reportText(locale, "Sugestões", "Suggestions"), provenance.suggestions.provider || unavailable(locale), provenance.suggestions.count ?? unavailable(locale), provenance.suggestions.endpoint || unavailable(locale)]);
  if (provenance?.ideas) rows.push([reportText(locale, "Ideias", "Ideas"), provenance.ideas.provider || unavailable(locale), provenance.ideas.count ?? unavailable(locale), provenance.ideas.endpoint || unavailable(locale)]);
  if (provenance?.serp) rows.push(["SERP", provenance.serp.provider || unavailable(locale), provenance.serp.keyword_count ?? unavailable(locale), reportText(locale, "keywords analisadas", "keywords analyzed")]);
  if (provenance?.hypothesis_only) rows.push([reportText(locale, "Modo hipótese", "Hypothesis mode"), yesNo(true, locale), unavailable(locale), provenance.hypothesis_reason || unavailable(locale)]);
...
function buildTopicClusterReportPayload(cluster: AnyRecord, localeInput?: unknown): AnyRecord {
  ...
  return {
    ...
    charts: [{
      ...
      type: "bar",
      height: "tall",
      data: { labels: pages.map((page: AnyRecord) => page.keyword_principal?.keyword || page.slug || reportText(locale, "página", "page")), datasets: [{ label: "Volume", data: pages.map((page: AnyRecord) => page.keyword_principal?.volume || 0), backgroundColor: REPORT_COLORS.blue }] },
      options: { indexAxis: "y", scales: { x: { beginAtZero: true } } },
    }],
    sections: [
...
async function commandCompetitiveAnalysis(args: AnyRecord): Promise<void> {
  if (!args.target) throw new CliError("Missing --target. Usage: competitive-analysis --target <domain|url> --competitors <a,b> [--mode domain|url|mixed] [--preset domain-full|quick|url-headtohead|content-only|brand-only|full] [--location-code 2076] [--language-code pt] [--keyword-limit 500] [--ctr-curve-id awr_2026_q2] [--attach-backlink-analysis-run slug] [--sample] [--confirm-budget] [--offline]");
  if (!args.competitors) throw new CliError("Missing --competitors (comma-separated, at least one).");
  ensureProject();
  const forwardArgs: string[] = [path.join(ROOT, "scripts", "competitive-analysis.mjs")];
  for (const key of Object.keys(args)) {
    if (key === "_") continue;
    const flag = `--${key.replace(/_/g, "-")}`;
    const value = args[key];
    if (value === true) forwardArgs.push(flag);
    else if (value === false) continue;
    else forwardArgs.push(flag, String(value));
  }
  const result = spawnSync(process.execPath, forwardArgs, { cwd: ROOT, env: process.env, encoding: "utf8", maxBuffer: 64 * 1024 * 1024, stdio: ["ignore", "pipe", "inherit"] });
  if (result.error) throw new CliError(result.error.message);
  const stdout = result.stdout || "";
  process.stdout.write(stdout.endsWith("\n") ? stdout : stdout + "\n");
  if (result.status !== 0) {
    try { const parsed = JSON.parse(stdout); throw new CliError(parsed.error || `competitive-analysis exited with status ${result.status}`); }
    catch (e) { if (e instanceof CliError) throw e; throw new CliError(`competitive-analysis exited with status ${result.status}`); }
  }
}
...
async function commandProjectBrowser(args: AnyRecord): Promise<void> {
  ...
  if (args.open_path || args.target_path) childArgs.push("--open-path", String(args.open_path || args.target_path));
  ...
  if (!args.foreground && !args.no_detach) childArgs.push("--detach");
...
async function commandProjectInit(args: AnyRecord): Promise<void> {
  ...
  for (const dir of ["brain", "contents", "web", "sources", "workbench", "artifacts", "audits", "keywords", "clusters", "eeat", REPORT_DIR_NAME, ".agentic-seo"]) mkdirp(path.join(p, dir));
...
function lintContentPublication(projectDir: string): AnyRecord[] {
  ...
  if (!fs.existsSync(contentsRoot)) return findings;
  for (const origin of fs.readdirSync(contentsRoot)) {
    ...
    for (const name of fs.readdirSync(originDir)) {
      ...
      for (const issue of issues) findings.push({ severity: "error", file: `contents/${origin}/${name}`, message: issue });
    }
  }
  return findings;
}
...
async function commandInternalLinks(args: AnyRecord): Promise<void> {
...
function clusterVolume(value: unknown): number | null {
  if (value === null || value === undefined || value === "") return null;
  const n = Number(value);
  return Number.isFinite(n) && n >= 0 ? n : null;
}

function clusterYamlFromLegacy(cluster: AnyRecord): AnyRecord {
  const pillarSlug = String(cluster.pillar?.slug || slugify(cluster.pillar?.keyword_principal?.keyword || cluster.seed || "")).trim();
  const planned = (Array.isArray(cluster.supporting_pages) ? cluster.supporting_pages : []).map((page: AnyRecord) => {
...
function renderClusterPlanningMarkdown(clusterYaml: AnyRecord): string {
  const planned = Array.isArray(clusterYaml.planned_satellites) ? clusterYaml.planned_satellites : [];
  const rows = planned.length
    ? planned.map((item: AnyRecord) => `| ${item.title || item.slug} | ${item.keyword || "—"} | ${item.intent || "—"} | ${item.volume ?? "—"} |`).join("\n")
...
function promoteClusterDraft(projectDir: string, slug: string, approvedBy: string): AnyRecord {
  ...
  if (!fs.existsSync(draftPath)) throw new CliError(`Cluster draft not found: ${path.relative(projectDir, draftPath)}`);
  const draft = readYaml(draftPath);
  if (draft.provenance?.hypothesis_only && !approvedBy) throw new CliError("Hypothesis-only cluster promotion requires an approver.");
  ...
  try {
    ...
    const synced = spawnSync(process.execPath, [path.join(ROOT, "scripts", "cluster-sync.mjs"), `--root=${projectDir}`, `--cluster=${slug}`], {
      cwd: ROOT,
      encoding: "utf8",
      env: process.env,
      maxBuffer: 20 * 1024 * 1024,
    });
    if (synced.status !== 0) throw new Error(synced.stderr || synced.stdout || "cluster-sync failed");
    ...
    if (backupCluster === null) {
      if (fs.existsSync(clusterPath)) fs.unlinkSync(clusterPath);
    } else {
      writeText(clusterPath, backupCluster);
    }
    if (fs.existsSync(archivePath) && !fs.existsSync(draftPath)) fs.renameSync(archivePath, draftPath);
    if (backupIndex !== null) writeText(brainIndex, backupIndex);
    if (backupSubpage !== null) writeText(brainSubpage, backupSubpage);
    else if (fs.existsSync(brainSubpage)) fs.unlinkSync(brainSubpage);
...
`);
  ...
  if (args.render_only) {
    const synced = spawnSync(process.execPath, [path.join(ROOT, "scripts", "cluster-sync.mjs"), `--root=${p}`], {
      cwd: ROOT,
      encoding: "utf8",
      env: process.env,
      maxBuffer: 20 * 1024 * 1024,
    });
    if (synced.status !== 0) throw new CliError(`cluster-sync failed: ${synced.stderr || synced.stdout}`);
...
function parseContentClusterArgs(args: AnyRecord): string[] {
...
function parseContentRoleArg(args: AnyRecord, clusters: string[]): AnyRecord | undefined {
  const raw = String(args.role || "").trim().toLowerCase();
  if (!raw || !["pillar", "satellite"].includes(raw) || clusters.length === 0) return undefined;
  return Object.fromEntries(clusters.map((cluster) => [cluster, raw]));
}

function readReviewPageRules(projectDir: string, draftText: string): AnyRecord {
  ...
  if (!pagePresent) failures.push("brain/review.md ausente");
  if (!reviewBacked) failures.push("brain/review.md sem regra editorial universal reconhecível");
  const firstTextSentence = cleanDraft
    .replace(/^---[\s\S]*?---\s*/, "")
    .replace(/^#\s+.+$/m, "")
    .split(/[.!?]\s+/)[0]
    ?.trim();
  if (!firstTextSentence || firstTextSentence.length < 20) failures.push("lead inicial curto ou ausente");
  const iaSlopTerms = ["crucial", "robust", "comprehensive", "nuanced", "fundamental", "significant"];
  for (const term of iaSlopTerms) {
    if (new RegExp(`\\b${escapeRegExp(term)}\\b`, "i").test(cleanDraft)) failures.push(`IA-slop detectado: ${term}`);
  }
  for (const pattern of [/vamos entender/i, /neste artigo/i, /como você pode ver/i]) {
    if (pattern.test(cleanDraft)) failures.push(`Voz Conversion-explainer detectada: ${pattern.source}`);
  }
  if (/—/.test(cleanDraft)) failures.push("em dash detectado em prosa pública");
  return {
...
function runContentPublicationCheck(
  brief: AnyRecord,
  projectDir: string,
  draftPath: string,
  checkPath: string,
  ...
  if (contractVersion !== 1) issues.push("missing contract_version: 1 frontmatter");
  if (clusters.length === 0) issues.push("missing clusters frontmatter");
  for (const cluster of missingClusters) issues.push(`cluster not found: ${cluster}`);
  const reviewRules = readReviewPageRules(projectDir, text);
  for (const failure of reviewRules.failures || []) {
    if (!String(failure).includes("brain/review.md sem regra editorial")) issues.push(`review: ${failure}`);
  }
  const targetWords = contentTargetWords(brief);
...
function writeApprovedContentDraft(
  ...
  appendOperationalLog(
    ...
    "draft",
    `Rascunho público de SEO escrito em artifacts por ${trigger}; ainda não publicado em project/contents.`,
...
async function commandContentSeo(args: AnyRecord): Promise<void> {
  ...
  if (phase === "brief") {
    ...
    if (requestedRole) brief.role = requestedRole;
  ...
  if (!fs.existsSync(effectiveCheckPath)) throw new CliError("Publication checks must pass before promotion.");
  ...
  const origin = String(brief.origin || "blog");
  if (!PUBLIC_CONTENT_ORIGINS.has(origin)) throw new CliError(`Invalid origin: ${origin}. Use blog, linkedin, podcast, or other.`);
  ...
  try {
    ...
    syncResults = clusters.map((cluster) => {
      const synced = spawnSync(process.execPath, [path.join(ROOT, "scripts", "cluster-sync.mjs"), `--root=${p}`, `--cluster=${cluster}`], {
        cwd: ROOT,
        encoding: "utf8",
        env: process.env,
        maxBuffer: 20 * 1024 * 1024,
    ...
    if (failedSync) throw new CliError(`cluster-sync failed for ${failedSync.cluster}: ${failedSync.stderr || JSON.stringify(failedSync.stdout)}`);
  } catch (err: any) {
    for (const file of rollbackFiles) restoreSnapshot(file, snapshots.get(file) ?? null);
