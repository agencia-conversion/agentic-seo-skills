...
function writeMarkdownReport(file: string, payload: AnyRecord): void {
...
function formatLogFileRefs(files: string[]): string {
  ...
  return files.map((f) => {
    ...
    if (normalized.startsWith("workbench/") || normalized.startsWith("artifacts/") || normalized.startsWith("relatorios/") || normalized.startsWith("../") || normalized.startsWith("sources/") || normalized.includes(".")) return f;
    return `[[${normalized}]]`;
  }).join(", ");
}
...
function buildTopicClusterReportPayload(cluster: AnyRecord, localeInput?: unknown): AnyRecord {
  ...
  return {
    ...
    sections: [
      ...
      { heading: reportText(locale, "Páginas do cluster", "Cluster pages"), body_markdown: reportTable([reportText(locale, "Papel", "Role"), "Slug", "Keyword", "Volume", reportText(locale, "Intenção", "Intent")], pages.map((page: AnyRecord) => [page.role || "pillar", page.slug || "", page.keyword_principal?.keyword || "", page.keyword_principal?.volume ?? unavailable(locale), page.serp_intent || unavailable(locale)])) },
...
async function commandProjectInit(args: AnyRecord): Promise<void> {
  ...
  for (const dir of ["brain", "conteudos", "web", "sources", "workbench", "artifacts", "audits", "keywords", "clusters", "eeat", "relatorios", ".agentic-seo"]) mkdirp(path.join(p, dir));
  for (const origem of PUBLIC_CONTENT_ORIGENS) mkdirp(path.join(p, "conteudos", origem));
  for (const moduleId of REPORT_MODULE_IDS) mkdirp(path.join(p, "relatorios", moduleId));
