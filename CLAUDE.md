...
**Project Subfolders.** Skill artifacts live under one folder per dimension per slug. Content workbench em `project/workbench/content/<slug>/`, content artifacts em `project/artifacts/contents/<slug>/`, content published flat em `project/conteudos/<origem>/<slug>.md` (sem subpasta por slug). Demais: `project/keywords/<seed>/`, `project/audits/<slug>/`, `project/clusters/<slug>/` (cluster autônomo com `cluster.yaml` ou `draft.yaml`), `project/eeat/<slug>/`. Canonical report pages live under `project/analises/<module>/<run-slug>/report.md`, render in the Web Companion, and are editable human presentation artifacts governed by the shared `page-report` skill; structured report fences use YAML `version: 1` for new reports, with JSON only as legacy compatibility. Raw evidence stays in `source_artifact` plus module evidence folders. The brain (`project/brain/`) is the only authorial knowledge layer; changes must be logged as decisions with evidence. See `AGENTS.md` § Brain Rules → Project Subfolders.

**Topic Clusters como espinha.** Topic Clusters são entidade central. O contrato completo está em `docs/specs/topic-clusters-contract.md` (contract_version 1, plugin 0.2). Cada cluster ativo tem (1) `project/clusters/<slug>/cluster.yaml` como fonte de verdade operacional (`pilar`, `planned_satellites[]`, `satelite_overrides`; sem `satelites[]` para publicados), (2) `project/brain/topic-clusters/<slug>.md` como subpágina autoral com tabela materializada entre sentinels HTML comment `<!-- BEGIN cluster-content-table:auto:v1:do-not-edit -->` / `<!-- END cluster-content-table:auto -->`, e (3) entrada no índice `project/brain/topic-clusters.md` (entre sentinels `cluster-index-table`). Conteúdos declaram afiliação via `clusters: [<slug>, ...]` no frontmatter (N:N) — fonte única de afiliação. Sync via `node scripts/cluster-sync.mjs [--cluster=<slug>] [--check]`; `cluster-doctor` para diagnóstico read-only; `cluster-rename` / `cluster-retire` para ops compostas; `install-cluster-sync-hook --apply` para o pre-commit. Web Companion dispara sync server-side em `POST /api/project/file`. A pasta legada `project/contents/<slug>/` foi removida do contrato. Pastas por slug em `conteudos/` (ex: `conteudos/blog/<slug>/`) não existem.
