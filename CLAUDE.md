...
**Topic Clusters como espinha.** Topic Clusters são entidade central. Cada cluster ativo tem (1) `project/clusters/<slug>/cluster.yaml` como fonte de verdade operacional, (2) `project/brain/topic-clusters/<slug>.md` como subpágina autoral com tabela de conteúdos projetada do YAML, e (3) entrada no índice `project/brain/topic-clusters.md`. Conteúdos declaram afiliação via `clusters: [<slug>, ...]` no frontmatter (N:N). A pasta legada `project/contents/<slug>/` foi removida do contrato. Pastas por slug em `conteudos/` (ex: `conteudos/blog/<slug>/`) não existem.
